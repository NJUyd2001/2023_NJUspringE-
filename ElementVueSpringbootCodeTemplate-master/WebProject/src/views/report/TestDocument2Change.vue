...
<template>
    ...
    Axios.post("http://localhost:9090/api/process/findByUID",JSON.stringify(this.useruid),{
                ...
                Axios.post("http://localhost:9090/api/testrecord/find",JSON.stringify(this.userpid),{
                headers:{
                  'content-type': 'text/plain'}
              }).then(ret=>{
                console.log(ret.data)
  ...
  window.addEventListener('unload', this.handleUnload);
    ...
    sessionStorage.setItem("store",JSON.stringify(this.$store.state))
        ...
        submitForm(formName) {
          Axios.post("http://localhost:9090/api/testrecord/update",JSON.stringify(this.ruleForm),{
                headers:{
                  'content-type': 'text/plain'}
              }).then(ret=>{
        console.log(ret.data);
